...
Now `left_join()` with the `point_metadata` table to aggregate our metadata in a single table: 
```{r}
point_metadata <- left_join(point_metadata, sp_richness, by = join_by(Name_Point == point_names))
head(point_metadata)
```
Voila! Now we can easily use the `point_metadata` table to analyze and visualize the relationships between the various metrics we are interested in! How does the habitat type (sun vs. shade) relate to the number of species at the point? How does canpoy cover play a role? How do we quantify those effects? While it seemed trivial at firest to count the species in each point, we have now developed a programatic approach that allows  
